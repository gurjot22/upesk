...
define( 'CAPRI_VERSION', '1.1.22' );

$vendor_file = trailingslashit( get_template_directory() ) . 'vendor/autoload.php';
if ( is_readable( $vendor_file ) ) {
	require_once $vendor_file;
}
add_filter( 'themeisle_sdk_products', 'capri_load_sdk' );
/**
 ...
 */
function capri_load_sdk( $products ) {
...
if ( ! function_exists( 'capri_setup' ) ) :
	...
	function capri_setup() {
		 ...
		 * to change 'capri-pro' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'capri-pro', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'primary' => esc_html__( 'Primary', 'capri' ),
			)
		);
...
function capri_widgets_init() {
	...
	if ( class_exists( 'WooCommerce' ) ) {
		register_sidebar(
			array(
				'name'          => esc_html__( 'Sidebar Shop', 'capri' ),
				'id'            => 'sidebar-shop',
				'before_widget' => '<div id="%1$s" class="widget %2$s">',
				'after_widget'  => '</div>',
				'before_title'  => '<h4 class="widget-title">',
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Ribbon area', 'capri' ),
			'id'            => 'ribbon-area',
			'class'         => 'ribbon-area',
		...
		sprintf(
			...
			sprintf(
				wp_kses(
					...
					__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'capri' ), array(
						'span' => array(
							'class' => array( '' ),
						),
					)
...
function capri_fonts_url() {
	 ...
	 */
	$bitter = _x( 'on', 'Bitter font: on or off', 'capri' );
	$arimo  = _x( 'on', 'Arimo font: on or off', 'capri' );
...
function capri_scripts() {
	...
	wp_localize_script(
		'capri-navigation', 'capriScreenReaderText', array(
			'expand'   => '<span class="screen-reader-text">' . esc_html__( 'expand child menu', 'capri' ) . '</span>',
			'collapse' => '<span class="screen-reader-text">' . esc_html__( 'collapse child menu', 'capri' ) . '</span>',
		)
	);
...
function capri_textarea_placeholder( $fields ) {
	$fields['comment_field'] = str_replace(
		'<textarea',
		'<textarea placeholder="' . esc_html__( 'Comment', 'capri' ) . '"',
